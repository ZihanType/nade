...
As you can see in [How it works](#how-it-works), the code generated by `#[nade]` contains calls to the macro `$crate::nade_helper!(..)`, so you have to import it in the root of crate, because the declarative macro `foo` may be used in other crate so it is recommended to use the `pub use` statement to re-export the macro.
